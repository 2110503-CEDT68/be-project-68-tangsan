...
exports.getReservations = async (req, res, next) => {
    try {
        ...
        if (req.user.role !== 'admin') {
            query = Reservation.find({ user: req.user.id }).populate({ path: 'restaurant', select: 'name address tel opentime closetime' });
        } else {
            query = Reservation.find().populate({ path: 'restaurant', select: 'name address tel opentime closetime' });
        }
...
exports.getReservation = async (req, res, next) => {
    try {
        const reservation = await Reservation.findById(req.params.id).populate({ path: 'restaurant', select: 'name address tel' });
...
exports.addReservation = async (req, res, next) => {
    try {
        ...
        if (!restaurant) return res.status(404).json({ success: false, message: `No restaurant with the id of ${req.params.restaurantId}` });

        // ตรวจสอบเงื่อนไข User จองได้สูงสุด 3 โต๊ะ
